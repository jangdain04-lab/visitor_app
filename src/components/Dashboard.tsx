...
export function Dashboard({ navigation }: any) {
  ...
  const childCards = useMemo(() => mockMissingChildren.slice(0, 2), []);
    ...
    <SafeAreaView style={styles.root} edges={['top']}>
      ...
      >
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.86}
          onPress={() => setAnnouncementListOpen(true)}
        >
          ...
          <View style={styles.noticeContent}>
            <Text style={[styles.noticeTitle, { fontSize: 16 * scale }]}>{latestAnnouncement.title}</Text>
            <Text style={[styles.noticeSummary, { fontSize: 15 * scale }]}>{latestAnnouncement.summary}</Text>
            <View style={styles.noticeFooter}>
              <Text style={[styles.noticeTime, { fontSize: 14 * scale }]}>{getTimeAgo(latestAnnouncement.timestamp)}</Text>
